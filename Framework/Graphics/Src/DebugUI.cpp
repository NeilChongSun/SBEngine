...
namespace
{
	...
	bool IsKeyBoradInput(UINT message)
	{
		switch (message)
		{
		case WM_KEYDOWN:
...
LRESULT CALLBACK DebugUIMessageHandler(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (io.WantCaptureKeyboard&&IsKeyBoradInput(message))
		return ImGui_ImplWin32_WndProcHandler(window, message, wParam, lParam);
...
void DebugUI::SetTheme(Theme theme)
{
	switch (theme)
	{
	case SB::Graphics::DebugUI::Theme::Classic:
		ImGui::StyleColorsClassic();
		break;
	case SB::Graphics::DebugUI::Theme::Dark:
		ImGui::StyleColorsDark();
		break;
	case SB::Graphics::DebugUI::Theme::Light:
