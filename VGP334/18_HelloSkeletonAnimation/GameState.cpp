...
void GameState::DebugUI()
{
	...
	ImGui::Text("Timer: %.2f", mAnimator.GetTimer());
	ImGui::End();
}
...
void GameState::DrawScene()
{
	...
	for (int i = 0; i < mModel.skeleton.bones.size(); i++)
	{
		boneTransfomrData.boneTransforms[i] = Transpose(mModel.skeleton.bones[i]->offsetTransform * mAnimator.GetBoneMatrices()[i]);
	}
	...
	if (!mIsShowSkeleton)
		mModel.Draw();
	if (mIsShowSkeleton)
		DrawSkeleton(mModel.skeleton, mAnimator.GetBoneMatrices(),matWorld);
	//-----------------------------------------------------------------------------------
