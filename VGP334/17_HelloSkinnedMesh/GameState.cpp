...
void GameState::Initialize()
{
	...
	{
		UpdatePose(bone.get(), mBoneMatrices);
	}
	...
	mPixeShader.Initialize("../../Assets/Shaders/Standard.fx");
...
void GameState::DrawScene()
{
	...
	BoneTransformData boneTransfomrData;
	for (int i = 0; i < mBoneMatrices.size(); i++)
	{
		//boneTransfomrData.boneTransforms[i] = mModel.skeleton.bones[i]->offsetTransform* mBoneMatrices[i];		
		boneTransfomrData.boneTransforms[i] = Transpose(mModel.skeleton.bones[i]->offsetTransform* mBoneMatrices[i]);		
	}

	mBoneTransformBuffer.Update(boneTransfomrData);
	mBoneTransformBuffer.BindVS(5);

	if (!mIsShowSkeleton)
		mModel.Draw();
	if (mIsShowSkeleton)
		DrawSkeleton(mModel.skeleton, mBoneMatrices,matWorld);
	//-----------------------------------------------------------------------------------
